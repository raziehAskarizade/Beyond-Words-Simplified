...
class AmazonReviewGraphDataModule(GraphDataModule):
    ...
    def __get_co_occurrence_graph(self):
        return CoOccurrenceGraphConstructor(self.df['Review'][:self.end_data_load], path.join(self.graphs_path, 'co_occ'),  load_preprocessed_data=True, naming_prepend='graph', start_data_load=self.start_data_load, end_data_load=self.end_data_load)
   
    def __get_sequential_graph(self):
        return SequentialGraphConstructor(self.df['Review'][:self.end_data_load], path.join(self.graphs_path, 'seq_gen'), load_preprocessed_data=True, naming_prepend='graph', start_data_load=self.start_data_load, end_data_load=self.end_data_load , use_general_node=True)
